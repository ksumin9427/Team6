...
public class M_lectureDBbean {
	...
	public ArrayList<M_lectureBean> listlecture(int l_no) throws Exception {
		...
		ArrayList<M_lectureBean> list = new ArrayList<M_lectureBean>();
		M_lectureBean mltb = null;
		String sql = "select l_no,l_name,l_level,l_sem,l_unit,l_max,l_day,l_start,l_time,l_com,l.major_no,professor_p_no,m.name \r\n" + 
				"from lecture l,major m \r\n" + 
				"where l.major_no = m.no and l_no=?"; 
		try {
			con = getConnection();
			ps = con.prepareStatement(sql);
			ps.setInt(1, l_no);
			...
			while (rs.next()) {
				...
				mltb.setMajor_no(rs.getInt(11));
				mltb.setProfessor_p_no(rs.getInt(12));
				mltb.setM_name(rs.getString(13));
				list.add(mltb);
			}
	...
	public int test(int p_no) {
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		String sql = "select major_no from professor where p_no =";
		int num=0;
		try {
			con = getConnection();
			ps = con.prepareStatement(sql);
