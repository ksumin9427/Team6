...
public class LectureDBBean {
	...
	//중복요일 확인
	public boolean confirmCal(int s_no,int l_start,String l_day) {
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		String sql="";
		try {
			con = getConnection();
			sql="select l_day,l_start from lecture l , lecture_a a where l.l_no=a.l_no and l_day=? and l_start=? and s_no=?";
			ps = con.prepareStatement(sql);
			ps.setString(1, l_day);
			ps.setInt(2, l_start);
			ps.setInt(3, s_no);
			rs=ps.executeQuery();
			if(rs.next()) {
