...
public class NoticeDBBean {
	...
	public int insertNotice(NoticeBean notice) {
		...
		try {
			...
			pstmt.setString(3, notice.getN_name());
			pstmt.setString(4, notice.getN_email());
			pstmt.setString(5, notice.getN_title());
			pstmt.setString(6, notice.getN_content());
...
//			stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE); 
	...
	public ArrayList<NoticeBean> listStudentNotice(String pageNumber) {
		...
		try {
			...
			stmt = con.createStatement();
			pageSet = stmt.executeQuery("select count(n_no) from notice where n_div='학생'"); //�븰�깮湲��쓽 媛쒖닔 援ы븯湲� 
	...
	public ArrayList<NoticeBean> listProfessorNotice(String pageNumber) {
		...
		try {
			...
			stmt = con.createStatement();
			pageSet = stmt.executeQuery("select count(n_no) from notice where n_div='교수'"); //援먯닔湲��쓽 媛쒖닔 援ы븯湲� 
